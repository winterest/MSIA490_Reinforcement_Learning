...
for episode in range(1):
    state_history += [env.reset()] # Reset environment and record the starting state
    done = False
...
#policy_net = AlexNet(num_classes=4).to(device)
#target_net = AlexNet(num_classes=4).to(device)
...
def optimize_model(maxq):
    ...
    #print(torch.max(state_action_values))
...
for i_episode in range(num_episodes):
    state = env.reset()
    state = preprocess(state-state)
    state = torch.from_numpy(state).type(torch.FloatTensor).view(1,1,80,80).cuda()
    maxq = -9999
    reward_sum = 0
    for t in count():
        last_state = state
